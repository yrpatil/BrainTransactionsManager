...
class PortfolioManager(KillSwitchMixin):
    ...
    def get_portfolio_summary(self, strategy_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        Returns:
            If strategy_name provided: summary for that strategy.
            If not: aggregated summary across all strategies including per-strategy breakdown.
        """
        try:
            query = """
                SELECT 
            ...
            # If a specific strategy was requested, return its single summary (or empty)
            if strategy_name:
                return results[0] if results else {}
            
            # Otherwise, build an aggregated view across all strategies
            if not results:
                return {
                    'strategies': [],
                    'totals': {
            ...
            # Normalize numeric types and aggregate
            def _to_float(value):
                try:
    ...
    def get_strategy_summary(self, strategy_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Holdings list for this strategy
            holdings_df = self.get_all_positions(strategy_name)
                ...
                for _, row in holdings_df.iterrows():
                    try:
                        holdings_list.append({
                            ...
                            "last_updated": row.get("last_updated").isoformat() if pd.notnull(row.get("last_updated")) else None,
                        })
                    except Exception:
                        # Fallback minimal row if conversion fails
                        holdings_list.append({
                            "ticker": row.get("ticker"),
                            "quantity": row.get("quantity"),
                            "avg_entry_price": row.get("avg_entry_price"),
                            "last_updated": str(row.get("last_updated")) if row.get("last_updated") is not None else None,
                        })
