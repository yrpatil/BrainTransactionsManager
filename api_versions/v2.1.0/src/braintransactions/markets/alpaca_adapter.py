...
class AlpacaAdapter(MarketAdapter):
    ...
    def _generate_client_order_id(self, symbol: str, strategy_name: str) -> str:
        ...
        # Normalize parts to safe characters and reasonable lengths
        ...
        try:
            ...
            for order in orders:
                try:
                    ...
                    if not hasattr(order, 'id') or not order.id:
                        logger.warning(f"Skipping order without ID: {order}")
                        continue
                    
                    asset_type = self._detect_asset_type(order.symbol)
                    
                    # Convert order status safely
                    try:
                        order_status = self._convert_order_status(order.status).value
                    except Exception as status_error:
                        logger.warning(f"Error converting order status: {status_error}")
